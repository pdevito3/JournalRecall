    ...
    IRefreshTokenStore store, IOptions<RefreshTokenOptions> options, TimeProvider timeProvider)
{
    ...
    /// on failure (unknown/expired/reused token) no token, and the caller must reject the refresh.</summary>
    public sealed record Rotation(bool Succeeded, Guid UserId, string? Token, DateTimeOffset? ExpiresAt, Guid ChainId)
    {
        public static Rotation Fail() => new(false, Guid.Empty, null, null, Guid.Empty);
        public static Rotation Ok(Guid userId, Issued issued) => new(true, userId, issued.Token, issued.ExpiresAt, issued.ChainId);
    }

    /// <summary>Issues the first token of a brand-new chain (called on login).</summary>
    public async Task<Issued> IssueAsync(Guid userId, string? deviceLabel, CancellationToken cancellationToken = default)
    {
        var (issued, _) = await MintAsync(userId, Guid.CreateVersion7(), deviceLabel, cancellationToken);
        await store.SaveChangesAsync(cancellationToken);
        return issued;
    }

    /// <summary>Rotates a presented token: validate → revoke current + mint successor in the same chain.
    /// Reuse outside the grace window revokes the whole chain.</summary>
    public async Task<Rotation> RotateAsync(string rawToken, string? deviceLabel, CancellationToken cancellationToken = default)
    {
        ...
        {
            var (issued, successorId) = await MintAsync(current.UserId, current.ChainId, deviceLabel, cancellationToken);
            current.MarkReplacedBy(now, successorId);
            await store.SaveChangesAsync(cancellationToken);
            return Rotation.Ok(current.UserId, issued);
        }
        ...
        if (current.WasRotated && current.RevokedAt is { } revokedAt)
        {
            ...
            {
                var (issued, _) = await MintAsync(current.UserId, current.ChainId, deviceLabel, cancellationToken);
                await store.SaveChangesAsync(cancellationToken);
                return Rotation.Ok(current.UserId, issued);
            }
    ...
    private async Task<(Issued Issued, Guid Id)> MintAsync(Guid userId, Guid chainId, string? deviceLabel, CancellationToken cancellationToken)
    {
        var now = timeProvider.GetUtcNow();
        var raw = Base64Url(RandomNumberGenerator.GetBytes(32)); // 256 bits of entropy
        var expiresAt = now + _options.InactivityWindow;
        var token = RefreshToken.Issue(userId, Hash(raw), chainId, now, expiresAt, deviceLabel);
