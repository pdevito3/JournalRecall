...
await runFlow('session-isolation', async (page) => {
  ...
  await expectText(rawEditor(page), alpha, { message: 'Session A shows ALPHA' })
  if ((await rawEditor(page).innerText()).includes(bravo)) throw new Error('Session A leaked BRAVO text')
  ...
  await expectText(rawEditor(page), bravo, { message: 'Session B shows BRAVO' })
  if ((await rawEditor(page).innerText()).includes(alpha)) throw new Error('Session B leaked ALPHA text')
