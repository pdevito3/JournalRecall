...
import { todayYmd, useGenerateSummary, useSummary } from '@/features/summaries/useSummaries'
...
export function SummariesPage({
  ...
  date: string
  onChange: (next: { period?: SummaryPeriod; date?: string }) => void
}) {
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        <label className="flex items-center gap-2 text-sm text-muted">
          ...
          <input
            type="date"
            value={picked}
            onChange={(e) => onChange({ date: e.target.value || todayYmd() })}
